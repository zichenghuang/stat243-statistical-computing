...
@
\newpage
\noindent Plot the comparison of the PIKK algorithm as follows. Line on top is original algorithm, line on bottom is the modified algorithm. 
...
@
\newpage
\noindent Plot the comparison of the FYKD algorithm as follows. Line on top is original algorithm, line on bottom is the modified algorithm. 
<<fig.width=6, fig.height=4>>=
